...
namespace mySTL {
    ...
    RBTree<Value, Compare, Alloc>::insert(const value_type& val) {
        ...
        while (node != 0) {
            par = node;
            node = compare(val, node->data) ? node->left : node->right;
        }
    ...
    RBTree<Value, Compare, Alloc>::insert_aux(node_pointer x, node_pointer y,
        ...
        if (par == header || node != nullptr || compare(val, par->data)) {
            par->left = new_node;
            if (par == header) {  //一个节点
                getRoot() = new_node;
                rightMost() = new_node;
            } else if (par == leftMost()) {  //比最左节点还小
                leftMost() = new_node;
            }
        } else {
            par->right = new_node;
